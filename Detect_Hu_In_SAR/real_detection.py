...
# Initialize camera feed (camera 0 for default webcam)
...
def process_camera_feed():
    """Process live camera feed for human detection."""
    ...
    while cam.isOpened():
        if time.time() - start_time > 60:
            print("Stopping camera feed after 1 minute.")
            break  # Stop processing after 1 minute

        ret, frame = cam.read()
        if not ret:
            break  # Stop if the frame is not captured properly
...
# Call the function to start the camera feed and human detection
process_camera_feed()
